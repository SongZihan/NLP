...
class MyDataSet(Data.Dataset):
    def __init__(self, src, tgt):
        ...
        self.src = torch.tensor(src,dtype=torch.int32)
        self.tgt = torch.tensor(tgt,dtype=torch.int32)
...
if __name__ == '__main__':
    ...
    with console.status("[bold green]Working on processing...") as status:
        ...
        result['train'] = {}
        result['train']['src'] = vocab.text_to_ids(train_df['source_language'].tolist(),max_length=ParameterStorage.max_sentence_length,IsDecoderData=False)
        ...
        result['train']['tgt'] = decoder_data.tolist()

        result['valid'] = {}
        result['valid']['src'] = vocab.text_to_ids(valid_df['source_language'].tolist(),max_length=ParameterStorage.max_sentence_length,IsDecoderData=False)
        decoder_data = vocab.text_to_ids(valid_df['target_language'],True,max_length=ParameterStorage.max_sentence_length)
        decoder_data = np.array(decoder_data)
        result['valid']['tgt'] = decoder_data.tolist()


        result['test'] = {}
        result['test']['src'] = vocab.text_to_ids(test_df['source_language'].tolist(),max_length=ParameterStorage.max_sentence_length,IsDecoderData=False)
        decoder_data = vocab.text_to_ids(test_df['target_language'],True,max_length=ParameterStorage.max_sentence_length)
        decoder_data = np.array(decoder_data)
        result['test']['tgt'] = decoder_data.tolist()

        #################### 测试正确性 ####################
        print("src:")
        print(f"RawText: {train_df['source_language'].tolist()[0]}")
        print(f"TranformText: {vocab.ids_to_single_text(result['train']['src'][0],IsDecoderData=False)}")
        print("tgt:")
        print(f"RawText: {train_df['target_language'].tolist()[0]}")
        print(f"TranformText: {vocab.ids_to_single_text(result['train']['tgt'][0])}")
